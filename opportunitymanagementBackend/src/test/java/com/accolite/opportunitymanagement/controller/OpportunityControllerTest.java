...
public class OpportunityControllerTest {
    ...
    public void getControllerTest() throws Exception{
        int id=1;
        Opportunity opportunity = setOpportunityObject();
        Mockito.when(opportunityServiceImpl.getOpportunity(id)).thenReturn(opportunity);
        mockMvc.perform((get("/opportunity/".concat(String.valueOf(id))))).andExpect(status().isOk());
    }
    @Test
    ...
    public void updateTest() throws Exception{
        ...
        String jsonString = objectMapper.writeValueAsString(opportunity);
       Mockito.when(opportunityServiceImpl.insert(opportunity)).thenReturn(1);
        mockMvc.perform(put("/opportunity/update/1")
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .content(jsonString)
                .accept(MediaType.APPLICATION_JSON_VALUE)
        ).andExpect(status().isOk());
    ...
    public void deleteTest() throws Exception{
        ...
        mockMvc.perform(delete("/opportunity/delete/".concat(String.valueOf(id)))
                .contentType(MediaType.APPLICATION_JSON_VALUE)
